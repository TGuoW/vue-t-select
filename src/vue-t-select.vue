<template>
  <div class="vue-t-select-w">
    ...
    <transition name="vue-t-transition">
      <ul
        v-if="state.includes('UNFOLD')"
        class="vue-t-select-dropdown"
      >
        <li
...
<style scoped>
.vue-t-transition-enter-active,
.vue-t-transition-leave-active {
  transition: all 0.2s;
}
.vue-t-transition-enter,
.vue-t-transition-leave-to {
...
.vue-t-select-dropdown::-webkit-scrollbar {
  width: 8px;
  height: 16px;
...
/*定义滑块 内阴影+圆角*/
.vue-t-select-dropdown::-webkit-scrollbar-thumb {
  border-radius: 10px;
  -webkit-box-shadow: inset 0 0 6px rgb(180, 180, 180);
